function runEstimator_sigma(id)
...
%Logging
diary OFF
diaryFileName = ['estimationOutput/estimation_rob_sigma' num2str(id) '.txt'];
...
% ME.commonParamNames{end+1} = 'sigmaInf';

gamma = gammaVals(2);
for iR = 1:ME.numRegions
    ME.regionModels{iR}.gamma = gamma;
    ME.regionModels{iR}.sigmaInf = 0.15;
end
disp('Gamma:');
disp(gamma);
